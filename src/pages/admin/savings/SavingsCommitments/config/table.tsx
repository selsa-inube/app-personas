...
const mapSavingAccountPayment = (movement: IEntry): IMovement => {
  return {
    id: movement?.id,
    date: movement?.date,
...
const savingAccountPaymentsNormalizeEntries = (movements: IMovement[]) =>
  movements.map((entry) => ({
    ...entry,
    date: entry.date && formatPrimaryDate(entry.date),
  }));

const savingsAccountPaymentsTableTitles = [
  {
    id: "date",
    titleName: "Fecha",
    priority: 0,
...
const savingsAccountPaymentsTableBreakpoints = [
...
const savingsAccountPaymentsTableActions: IAction[] = [
  {
    id: "1",
    actionName: "Valor",
    content: (movement) => (
  ...
  },
  {
    ...
    actionName: "Ver",
    content: (movement) => (
      <ViewSavingMovement movement={mapSavingAccountPayment(movement)} />
    ),
    mobilePriority: true,
  },
];
